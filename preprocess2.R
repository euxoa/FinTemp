...
if (F) {
  ...
  ranef(m)$subsid["(Intercept)"] # Outliers, FIXME: make to filter automatically on ntercept and trend 
}

too.short <- (t2 %>% group_by(subsid) %>% summarise(n=n()) %>% filter(n<12*15))$subsid
t2 <- t2 %>% filter(! (subsid %in% c("10490000.1336", "10920000.4037", "22260000.4257", "10920000.6340")) )
t2 <- t2 %>% filter(! (subsid %in% too.short))
...
if (F) {
  ...
  t2 %>%  ggplot(aes(x=year, y=t, color=name)) + geom_point() + facet_wrap(~ month)
  t2 %>%  ggplot(aes(x=year, y=t, color=month)) + geom_point() + facet_wrap(~ name)
  ...
  acf((t2 %>% mutate(r=resid(m4)) %>% group_by(year, imonth) %>% 
         summarise(r=mean(r)) %>% mutate(ym=year+imonth/12) %>% arrange(ym))$r)
